...
screen_gaze_hist <- combined_n %>%
  ...
  ggplot(aes(x = comb_gaze_x, comb_gaze_y, fill = ..count../sum(..count..)*100,
...
fix_df %>%
  ...
  geom_errorbar(aes(ymin = fix_median_comb_pupil_size - fix_sd_comb_pupil_size,
                ...
                width = 0.7,
                size = 1) +
  geom_linerange(aes(ymin = fix_min_comb_pupil_size,
                    ymax = fix_max_comb_pupil_size),
                color = 'gray10') +
...
Median combined pupil size are plotted as functions of gaze position during stimulus blocks. Only positions within the boundary of the stimulus area are plotted. Non-normalized median values are plotted first followed by fixation normalized values.
...
stim_gaze_pupil_size <- combined_n %>%
  ...
  labs(title = 'Median Pupil Size', x = '', y = '', fill = 'Pupil Size') +
  c.theme()
