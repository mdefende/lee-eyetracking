...
hexbin_count <- function(df, 
  ...
  hbin <- hexbin(df[[{{ gaze_x }}]],df[[{{ gaze_y }}]], xbins = nbins, IDs = TRUE)

  data.frame(hcell2xy(hbin),
             cell = hbin@cell,
             count = hbin@count) %>%
...
full_screen_hexbin <- combined_n %>%
  filter(block != 'response', between(comb_gaze_x,0,1920) & between(comb_gaze_y,0,1080)) %>%
  hexbin_count()

screen_gaze_hist <- full_screen_hexbin %>%
  ggplot(aes(x = x, y = y, fill = ncount*100,
             text = paste('x:', round(x,0), '\n',
                          'y:', round(y,0), '\n',
                          'percent:', round(ncount*100,2),'%'))) +
